...
public class Template2CustomerSurveyPage extends AppPage {
	...
	public void rateResponsive(int rate) {
		waiting(2000);
		int slider_rail_width = driver.findElement(By.cssSelector("[class='slider-item']")).getSize().width; // אלמנט של
																												// אורך
																												// הסליידר
		actions.clickAndHold(driver.findElement(By.cssSelector("[type='range']"))).pause(Duration.ofMillis(400)).build()
				.perform();// העיגול שתופסים ומזיזים את הסליידר
		actions.moveByOffset(slider_rail_width / 10 * rate - 6, 0).build().perform();
		actions.release().build().perform();
	}
